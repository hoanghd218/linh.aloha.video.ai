...
Khách điền form đăng ký bất kỳ trang nào → POST /api/register
     ↓  route đọc cookie aff_ref phía server → lưu leads.aff_code
Khách chuyển khoản → Sepay webhook → markOrderPaid()
...
Phase 5: PATCH 4 file có sẵn (leads-supabase / register / sepay-webhook / layout)
