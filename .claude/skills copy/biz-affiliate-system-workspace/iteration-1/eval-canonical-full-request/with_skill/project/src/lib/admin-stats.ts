// lib/admin-stats.ts
//
// Dashboard analytics — KPI snapshot + time-series + deltas vs kỳ trước.
// Aggregate in-process từ bảng `leads`. Timezone Asia/Ho_Chi_Minh (UTC+7).
...
type LeadRow = {
  ...
  created_at: string;
  paid_at: string | null;
  payment_record: { transferAmount?: number } | null;
};
...
function leadRevenue(l: LeadRow): number {
  ...
  return Number(l.payment_record?.transferAmount ?? l.amount ?? 0);
}
...
export async function getDashboardData(days: PeriodDays = 30): Promise<DashboardData> {
  ...
  const { data: rows, error } = await supabase
    .from("leads")
    .select("order_id, name, email, phone, ticket, amount, status, created_at, paid_at, payment_record")
