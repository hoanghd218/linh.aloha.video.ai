// app/admin/page.tsx
//
// 1CRM-inspired admin dashboard cho Agent Camp.
// Sidebar nav (Tổng quan / Khách hàng / Email marketing) + popup nhập pass.
// Charts qua recharts, email marketing qua /api/admin/campaigns.
...
// Mẫu khách dùng khi preview email (placeholder {{name}}/{{email}}).
...
type DashboardData = {
  ...
  timeseries: Array<{ date: string; leads: number; paid: number; revenue: number }>;
...
type Tab = "dashboard" | "leads" | "campaigns";
  ...
  return (
    <div style={S.gateWrap}>
      <form onSubmit={submit} style={S.gateCard}>
        ...
        <h2 style={S.gateTitle}>Admin Agent Camp</h2>
        <p style={S.gateSub}>Nhập mã để xem dashboard và quản lý chiến dịch email.</p>
        <input
          type="password"
...
function Sidebar({
  ...
  const items: Array<{ key: Tab; label: string; icon: string }> = [
    ...
    { key: "leads", label: "Khách hàng", icon: "👥" },
    { key: "campaigns", label: "Email marketing", icon: "✉️" },
  ];
  return (
    <aside style={S.sidebar}>
      <div style={S.brand}>
        <div style={S.brandDot}>AC</div>
        <div>
          <div style={S.brandName}>Agent Camp</div>
...
function DashboardTab({ pass }: { pass: string }) {
  ...
  return (
    <>
      <PageHeader
        title="Tổng quan Agent Camp"
        subtitle="Theo dõi leads, doanh thu và tỷ lệ chuyển đổi"
      >
                  ...
                  >
                    ...
                    <Tooltip
                      formatter={(value, name) => {
                        ...
                        if (name === "Doanh thu") return [formatVND(v), name];
            ...
            <Card title="Trạng thái" style={{ flex: 1, minWidth: 260 }}>
              <div style={{ width: "100%", height: 240 }}>
                <ResponsiveContainer>
                  <PieChart>
                    <Pie
                      data={[
                        { name: "Đã thanh toán", value: data.statusBreakdown.paid },
                        { name: "Chưa thanh toán", value: data.statusBreakdown.pending },
              ...
              <div style={S.totalsBox}>
                <div style={S.totalsRow}>
                  <span style={S.totalsLabel}>Tổng lũy kế</span>
                ...
                <div style={S.totalsRow}>
                  ...
                  <span style={S.totalsValue}>{formatVND(data.totals.allTimeRevenue)}</span>
                </div>
              </div>
            </Card>
          </div>
              ...
              <tbody>
                {data.leads.map((l) => (
                  <tr key={l.orderId} style={S.tr}>
                    ...
                    <td style={S.td}>
                      <span style={{ ...S.badge, ...statusBadgeStyle(l.status) }}>
                        {l.status === "paid"
                          ? "Đã thanh toán"
                          : l.status === "pending"
                            ? "Chưa thanh toán"
                    ...
                    <td style={S.tdMuted}>{l.paidAt ? formatDateTime(l.paidAt) : "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function CampaignsTab({ pass }: { pass: string }) {
  ...
  // HTML preview đang mở trong modal (null = đóng).
  ...
  async function submit(e: FormEvent) {
    ...
    if (
      !confirm(
        `Gửi email "${subject}" tới ${audienceLabel(audienceKind, audienceDays)}?\n\nKhông thể huỷ sau khi bắt đầu gửi.`,
      )
    )
      return;
    ...
    try {
      ...
      if (!res.ok) throw new Error(json.message || json.error || "Lỗi gửi chiến dịch");
      setFlash(
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Soạn và gửi chiến dịch email cho khách Agent Camp"
      />
      ...
      <div style={S.campaignGrid}>
          ...
          <form onSubmit={submit} style={S.form}>
            <Field label="Tên chiến dịch (nội bộ)">
              <input
                ...
                placeholder="Khuyến mãi giai đoạn 2"
                style={S.input}
              />
            </Field>
            <Field label="Tiêu đề email (khách thấy)">
              <input
                ...
                placeholder="Anh/chị giữ chỗ Agent Camp tháng 5 nhé"
                style={S.input}
              />
            </Field>
            <Field label="Tên người gửi (tuỳ chọn)">
              <input
                ...
                placeholder="Đội ngũ Agent Camp"
                style={S.input}
              />
            </Field>
            <Field label="Đối tượng nhận">
              <div style={S.radioGroup}>
                {[
                  ...
                  { v: "pending", label: "Chưa thanh toán (nhắc chuyển khoản)" },
                  ...
                  <label key={opt.v} style={S.radioLabel}>
                    <input
                      ...
                      onChange={() => setAudienceKind(opt.v as typeof audienceKind)}
                    />
                    {opt.label}
                  </label>
                ))}
            ...
            <Field
              ...
              hint={
                bodyFormat === "html"
                  ? "Dán mã HTML. {{name}} để chèn tên khách. Đoạn HTML rời sẽ tự bọc khung email; HTML đầy đủ (<html>) giữ nguyên."
            ...
            >
              ...
              <textarea
                ...
                placeholder={
                  bodyFormat === "html"
                    ? '<p>Chào {{name}},</p>\n<p>Em vừa cập nhật chương trình Agent Camp tháng 5…</p>\n<p><a href="https://...">Xem chi tiết</a></p>'
                    : "Chào {{name}},\n\nEm là Hoàng — em vừa cập nhật chương trình Agent Camp tháng 5...\n\nLink chi tiết: https://..."
                }
                style={{
                  ...S.textarea,
                  ...(bodyFormat === "html"
        ...
        <Card title="Lịch sử chiến dịch" style={{ flex: 1.4, minWidth: 360 }}>
          {loading ? (
            ...
            <div style={S.empty}>Chưa có chiến dịch nào. Soạn bên trái để bắt đầu.</div>
          ) : (
            <div style={{ overflow: "auto" }}>
              <table style={S.table}>
                <thead>
                ...
                <tbody>
                  {campaigns.map((c) => (
                    ...
                    >
                      ...
                      <td style={S.td}>
                        <span style={{ ...S.badge, ...campaignStatusStyle(c.status) }}>
        ...
        <div style={S.modalOverlay} onClick={() => setPreviewHtml(null)}>
          <div style={S.modalCard} onClick={(e) => e.stopPropagation()}>
            <div style={S.modalHeader}>
              <span style={{ fontWeight: 700, fontSize: 14 }}>Xem trước email</span>
...
function DeltaBadge({ pct, points }: { pct?: number; points?: number }) {
  ...
  const up = v > 0;
  const formatted =
    pct !== undefined ? `${Math.abs(v).toFixed(1)}%` : `${Math.abs(v).toFixed(1)}đ`;
  return (
    <span
      style={{
        ...S.delta,
...
const S: Record<string, React.CSSProperties> = {
  ...
  tdMuted: { padding: "12px 12px", color: MUTED, whiteSpace: "nowrap", fontSize: 12 },
  ...
  campaignGrid: { display: "flex", gap: 14, flexWrap: "wrap", alignItems: "flex-start" },
