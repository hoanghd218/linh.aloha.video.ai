...
export async function listAffiliateReferrals(affCode: string): Promise<AffiliateReferral[]> {
  ...
  const { data, error } = await getSupabaseAdmin()
    .from("leads")
    .select("name, phone, ticket, amount, status, created_at")
    .eq("aff_code", code)
    .order("created_at", { ascending: false });
  if (error) { console.error("listAffiliateReferrals failed:", error.message); return []; }
  return ((data ?? []) as Array<{ name: string; phone: string | null; ticket: string | null; amount: number | null; status: ReferralStatus; created_at: string }>)
    .map((r) => ({ name: r.name, phoneMasked: maskPhone(r.phone ?? ""), ticket: r.ticket ?? "", amount: Number(r.amount) || 0, status: r.status, createdAt: r.created_at }));
}
```
...
export async function recordCommissionForOrder(orderId: string): Promise<CommissionEarnedInfo | null> {
  ...
  const { data: lead, error: leadErr } = await supabase
    .from("leads").select("order_id, name, ticket, amount, aff_code").eq("order_id", orderId).maybeSingle();
  ...
  const { error } = await supabase.from("affiliate_commissions").insert({
    ...
    customer_name: lead.name, ticket: lead.ticket, order_amount: orderAmount,
  ...
  return {
    ...
    ticket: (lead.ticket as string) ?? "", orderAmount, commissionRate: rate, commissionAmount,
  };
}
```
