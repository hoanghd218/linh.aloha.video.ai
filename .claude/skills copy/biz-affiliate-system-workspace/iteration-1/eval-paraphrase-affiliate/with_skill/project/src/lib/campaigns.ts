// lib/campaigns.ts
//
// Email campaign CRUD + audience query cho admin email marketing tab.
...
export async function queryAudience(audience: Audience): Promise<AudienceLead[]> {
  ...
  else if (audience.kind === "all") q = q.in("status", ["paid", "pending"]);
  // last_days: bao gồm cả paid + pending, filter theo created_at

  if (audience.kind === "last_days" && audience.days) {
    const since = new Date(Date.now() - audience.days * 86400 * 1000).toISOString();
    q = q.gte("created_at", since).in("status", ["paid", "pending"]);
  }
...
export async function logCampaignSend(send: {
  campaignId: string;
