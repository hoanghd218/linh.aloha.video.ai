...
// POST /api/admin/campaigns                  → tạo + gửi đồng bộ
//   body: { name, subject, body, bodyFormat?, audience: { kind, days? }, fromName? }
//   bodyFormat ∈ 'text' (mặc định) | 'html'
...
export async function POST(request: Request) {
  ...
  if (!subject) return Response.json({ error: "missing_subject" }, { status: 400 });
  if (!text) return Response.json({ error: "missing_body" }, { status: 400 });
  if (!audience) return Response.json({ error: "invalid_audience" }, { status: 400 });
  ...
  try {
    ...
    return Response.json(
      { error: "audience_query_failed", message: err instanceof Error ? err.message : "Unknown" },
      { status: 500 },
    );
  }
  ...
  // Render 1 lần — placeholder {{name}}/{{email}} giữ nguyên, personalize theo từng khách.
  ...
  const campaign = await createCampaign(
    { name, subject, bodyHtml: templateHtml, bodyText: templateText, audience },
    recipients.length,
  );
  ...
  for (let i = 0; i < recipients.length; i++) {
    ...
    const personalizedHtml = personalize(templateHtml, { name: r.name, email: r.email });
    const personalizedText = personalize(templateText, { name: r.name, email: r.email });
