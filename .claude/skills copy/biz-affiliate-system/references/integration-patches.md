...
**1a. Thêm field vào `CreateLeadInput`** — tìm `export type CreateLeadInput`:
...
**1b. Ghi `aff_code` vào row insert** — tìm `const row = {` bên trong `createLead`,
...
Không cần đụng `LeadRow` hay `rowToOrder` — `recordCommissionForOrder()` tự query
bảng `leads` để đọc `aff_code`, không đi qua type `Order`.

---

## Patch 2 — `api/register/route.ts` (đọc cookie aff, gắn vào lead)
...
export async function POST(request: Request) {
  /* …parse body… */
  const affCode = readAffCookie(request);          // ← THÊM (gần đầu handler)
  /* … */
  const lead = await createLead({
    orderId: candidate,
    name, phone, email, ticket, amount,
...
**Mục tiêu**: sau khi `markOrderPaid()` thành công → tạo bản ghi hoa hồng.
...
**3b. Gọi sau khi mark paid thành công** — tìm chỗ `markOrderPaid()` trả về
thành công (thường là biến `paid` / `order` sau check `if (!paid)`), chèn:
...
phòng thủ thứ hai. Đặt nó **trước hoặc sau** `sendP5PostPayment()` đều được —
